"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.WorkspaceLoader = void 0;
const common_1 = require("@nestjs/common");
const workspace_entity_1 = require("../entities/workspace.entity");
const workspace_service_1 = require("../workspace.service");
const nestjs_graphql_dataloader_1 = require("nestjs-graphql-dataloader");
let WorkspaceLoader = class WorkspaceLoader extends nestjs_graphql_dataloader_1.OrderedNestDataLoader {
    constructor(workspaceService) {
        super();
        this.workspaceService = workspaceService;
        this.getOptions = () => ({
            query: (keys) => this.workspaceService.findByIds(keys),
        });
    }
};
WorkspaceLoader = __decorate([
    common_1.Injectable(),
    __metadata("design:paramtypes", [workspace_service_1.WorkspaceService])
], WorkspaceLoader);
exports.WorkspaceLoader = WorkspaceLoader;
//# sourceMappingURL=workspace.loader.js.map